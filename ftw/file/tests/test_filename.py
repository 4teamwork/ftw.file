...
class TestFileName(TestCase):
    ...
    def test_whitespace(self):
        ...
        self.assertEqual('attachment; filename*=UTF-8\'\'ein%20file.doc',
                         self.context.REQUEST.RESPONSE.getHeader(
                             'Content-disposition'),)

    def test_umlauts(self):
        self.get_response_for(filename='Gef\xc3\xa4hrliche Zeichen.doc')
        self.assertEqual(
            'attachment; filename*=UTF-8\'\'Gef%C3%A4hrliche%20Zeichen.doc',
            self.context.REQUEST.RESPONSE.getHeader('Content-disposition'))
